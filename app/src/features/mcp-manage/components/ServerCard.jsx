 ...
 */
export function ServerCard({ server, isBusy, onRefresh, onDisconnect, onToggleEnabled }) {
  return (
    <Card
      className={cn(
      ...
      <CardContent className="flex items-center gap-4 p-4">
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          ...
          {server.status === "error" && server.status_message ? (
            <span className="truncate text-xs text-destructive" title={server.status_message}>
