...
export function McpScreen() {
  ...
  async function handleConnect(payload) {
    ...
    if (server.status === "connected") {
      toast.success(`${server.name} connected`, {
        description: `${server.tool_count} tool${server.tool_count === 1 ? "" : "s"} added to your gateway.`,
      });
    } else {
      // Saved but unreachable: say so plainly rather than a bare success toast.
      toast.warning(`${server.name} saved, but not reachable`, {
        description: server.status_message || "Check the URL and credentials, then retry.",
      });
    }
  }
  ...
  async function handleToggleEnabled(id, enabled) {
    try {
      ...
      toast.error("Could not update the server", { description: error.message });
    }
  }
